...
msft_stock_data = pd.read_csv('/Users/eminn/PycharmProjects/stockPricePredictions/data/MSFT.csv')

"""
This code block formats the msft_stock_data
...
"""
msft_stock_data = msft_stock_data[['Date', 'Open', 'Close']]
msft_stock_data['Date'] = pd.to_datetime(msft_stock_data['Date'].apply(lambda x : x.split()[0]))
msft_stock_data.set_index('Date', drop=True, inplace=True)
# print(msft_stock_data.head())

"""
method to print the real-data opening
stock prices of MSFT in a graph.
...
def printInitialOpenStockPrices():
    ...
    ax[0].plot(msft_stock_data['Open'], label='Open', color='green')
...
def printInitialCloseStockPrices():
    ...
    chart_type = FigureCanvasTkAgg(figure, window)
    chart_type.get_tk_widget().pack();
    ax[1].plot(msft_stock_data['Close'], label='Close', color='blue')
...
MMS = MinMaxScaler()
msft_stock_data[msft_stock_data.columns] = MMS.fit_transform(msft_stock_data)
print(msft_stock_data.shape)

training_size = round(len(msft_stock_data) * 0.80) #Selecting 80% of the data for training and leaving the remaining for testing
train_data = msft_stock_data[:training_size] #The specific data to use to train the model
test_data = msft_stock_data[training_size:]  #the specific data to use to test the model
...
ms_slice_data = pd.concat([msft_stock_data.iloc[-202:].copy(),pd.DataFrame(test_inverse_predicted,columns=['open_predicted','close_predicted'],index=msft_stock_data.iloc[-202:].index)], axis=1)

ms_slice_data[['Open', 'Close']] = MMS.inverse_transform(ms_slice_data[['Open', 'Close']])
print(ms_slice_data.head())
"""
...
comparison.
"""
ms_slice_data[['Open', 'open_predicted']].plot(figsize=(10,6))
...
comparison
"""
ms_slice_data[['Close', 'close_predicted']].plot(figsize=(10,6))
plt.xticks(rotation=45)
plt.xlabel('Date', size=15)
