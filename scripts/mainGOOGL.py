...
google_stock_data = pd.read_csv('/Users/eminn/PycharmProjects/stockPricePredictions/data/GOOGL.csv')

"""
This code block formats the google_stock_data
...
def printInitialCloseStockPrices():
    ...
    chart_type = FigureCanvasTkAgg(figure, window)
    chart_type.get_tk_widget().pack();
    ax[1].plot(google_stock_data['Close'], label='Close', color='blue')
...
training_size = round(len(google_stock_data) * 0.80) #Selecting 80% of the data for training and leaving the remaining for testing
train_data = google_stock_data[:training_size] #The specific data to use to train the model
test_data = google_stock_data[training_size:]  #the specific data to use to test the model
